...
def seq_gen_ha(len_seq, data_seq, offset, n_his, n_pred, n_dcpair, day_slot):
...
def read_data_ha(file_path, data_config, n_dcpair, n_his, n_pred, day_slot):
    ...
    seq_test = seq_gen_ha(n_test, data_seq, n_train + n_val, n_his, n_pred, n_dcpair, day_slot)

    return seq_test

def model_test_ha(data, n_his, n_pred, results_file):
    test_data = data
    ...
    for index in range(len_seq):
        predict_Y[index, :, 0] = np.mean(test_X[index, :, 0:n_his], axis=1)
...
if __name__ == '__main__':
    ...
    data = read_data_ha(data_file, data_config, n_dcpair, n_his, n_pred, day_slot)
    print('Inference:')
    start = time.time()
    model_test_ha(data, n_his, n_pred, results_file)
    end = time.time()
    run_time = end - start
